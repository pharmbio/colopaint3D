...
def corr_between_non_replicates(df, method='spearman'):
    ...
    null_corrs = (shuffled_df
                  .groupby(['Metadata_cmpd_conc', 'Metadata_name', 'Metadata_cell_line'])
                  .apply(lambda g: median_corr_by_group(g, method=method)))

    return null_corrs.to_frame(name='null_corr')

def corr_between_replicates(df, method='spearman'):
