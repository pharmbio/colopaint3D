...
| **`Prepare_Slice_Features` rebuilds `normalized_data_merged_HCT116.csv` with 779 selected features where the shipped file has 781**, which moves Fig 2g's variance explained (PC1 22.78→21.16%, PC2 15.49→13.22%) | **resolved by rebuilding.** It now runs by default and Fig 2g is drawn from `data/exp1_main`; the 781-feature table is kept beside it as `*.as_shipped.csv`. The two dropped features are `AreaShape_BoundingBox{Area,Maximum_X}_cytoplasm`; the cause is *not* the feature dump — 011225 and 150125 both yield 779 — nor the blocklist, variance threshold (freq₂/freq₁ 0.89 and 0.70, far above the 0.05 cut) or a correlation tie among retained features (max \|r\| 0.49, 0.56). It is the same class as the Fig 4 metrics: the shipped table was written by a code/environment state that no longer exists. **The result is unaffected** — the slice effect still goes η²=0.64, r=+0.76 → η²=0.05, r=−0.12 |
