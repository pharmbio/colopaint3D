...
def main():
    ...
    for p in plates:
        ...
        neg_contr = sc_df.filter(pl.col("Metadata_cmpdName") == "[DMSO]")
        ...
        comp_list = list(sc_df["Metadata_cmpdName"].unique())
        for comp in tqdm.tqdm(comp_list, desc = "Processing compounds"):
            if comp == "[DMSO]":
                continue
            print("Analysing compound", comp)
            start_time = time.time()
            subset_df = sc_df.filter(pl.col("Metadata_cmpdName") == comp)
        ...
        sc_df.to_pandas().to_parquet(f"/home/jovyan/share/data/analyses/benjamin/Single_cell_project_rapids/grit_parquet/sc_features_{p}.parquet")
        all_sc_grit_df2.to_parquet(f"/home/jovyan/share/data/analyses/benjamin/Single_cell_project_rapids/grit_parquet/sc_grit_{p}.parquet")
        sampled_neg_contr.to_pandas().to_parquet(f"/home/jovyan/share/data/analyses/benjamin/Single_cell_project_rapids/grit_parquet/neg_control_features_{p}.parquet")
