...
`run_all.py --verify` checks that panels, manifest rows and source tables agree with
each other. It cannot see a panel that saved an *empty page* — which is exactly what
Fig5d did: healthy manifest row, correct 630-row source table, blank PDF. This is the
missing half of that check.

    python scripts/check_panels_nonblank.py
"""
